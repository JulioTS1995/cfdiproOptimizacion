		...
		<div class="row">
			...
			<br>
				<div id="encabezadoform" style="text-align:center">
				  <h2 class="font-weight-bold" style="text-align: center;color:#0059b3; line-height: 100px;">Formato Concentrado Global</h2>
				</div>
				<br>
				<center>
					<form method="post" action="FormatoConcentradoGlobal.php" enctype="multipart/form-data">

					<div class="form-group">
					<div>
					<label>Cliente:</label>
						<select class="form-control inputdefault" name="cliente" id="cliente" required aria-required="true">
							<option value='1549261'>COMERCIALIZADORA ELORO</option>
						<?php
				
				require_once('cnx_cfdi2.php');
				mysqli_select_db($cnx_cfdi2,$database_cfdi);

						$resSQL = "SELECT ID,RazonSocial FROM ".$_GET["prefijodb"]."Clientes WHERE ID <> '1549261' ORDER BY RazonSocial";
					...
					if (!$runSQL) {//debug
						...
						<select class="form-control inputdefault" name="circuito" id="circuito" required aria-required="true">
							<option value='0'>Seleccione Circuito</option>
						<?php
				
				require_once('cnx_cfdi2.php');
				mysqli_select_db($cnx_cfdi2,$database_cfdi);

						$resSQL = "SELECT ID,Nombre FROM ".$_GET["prefijodb"]."circuito WHERE Estatus = 'Activo' ORDER BY Nombre";
					$runSQL = mysqli_query($cnx_cfdi2,$resSQL);
					if (!$runSQL) {//debug
						$mensaje  = 'Consulta no válida: ' . mysql_error() . "\n";
						$mensaje .= 'Consulta completa: ' . $resSQL;
						...
						{
							?>
							<option value='<?php echo $rowSQL['ID']; ?>'><?php echo $rowSQL['Nombre']; ?></option>
						<?php
...
<!-- http://67.205.112.109/cfdipro/colosos_facturas_reporte_fechas.php?prefijodb=colosos -->