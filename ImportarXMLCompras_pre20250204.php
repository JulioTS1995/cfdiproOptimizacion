 ...
 if($value)
 {
  ...
  if($filename[1] == ('xml')||('XML')){
    ...
    foreach ($xml2->xpath('//t:TimbreFiscalDigital') as $Complemento) {
        ...
        if ($tipoComprobante!='I') {//debug
            echo "<script>alert('Este documento no es de tipo ingreso. Serie ".$serieF." y Folio: ".$folio."');</script>";
            //die('<h1>Intenta otra vez con otro XML</h1>');
        }

        $query5 = "SELECT * FROM ".$prefijobd."Compras WHERE Comentarios ='".$UUID."';"; 
    ...
    foreach ($xml2->xpath('//cfdi:Emisor') as $Emisor) {
        ...
        if($numRenglones==0){
            echo "<script>alert('No existe un proveedor con el RFC: ".$RFCEmisor.". \\nCon nombre: ".$NombreEmisor.".');</script>";
			//die("Registre al proveedor: ".$NombreEmisor." e intente de nuevo");
        }
        ...
        foreach ($xml2->xpath('//cfdi:Conceptos//cfdi:Concepto') as $Concepto) {
            ...
            if($numRenglones!=0 && $tipoComprobante=='I'){
                    ...
                    ('$new2id','$cantidad','$descripcion','$importe','$valorUnitario','Compras','$idPrincipal','$descuento','".$IVA[$cont3]."','".$importeIVA[$cont3]."','".$Retencion[$cont3]."','".$importeRetencion[$cont3]."');";
						//die($queryP);
...
echo "<script>alert('Importacion Exitosa, se creo la compra: ".$xfolio."');</script>";
}//Imprime exito
   }//fin foreach File[]
