...
?>
	...
	<?php
		...
		$resSQL11="SELECT DISTINCT(C.ID) as id_cliente FROM ".$prefijobd."factura F, ".$prefijobd."oficinas O, ".$prefijobd."clientes C WHERE F.CobranzaSaldo > 0 AND F.Oficina_RID = O.ID {$ctnPortalTotal} AND (F.cCanceladoT IS NULL OR F.cCanceladoT = '') AND F.CargoAFactura_RID = C.ID AND F.cfdfchhra > '1990-01-01 00:00:00' ORDER BY C.RazonSocial";
			//echo "<br>".$resSQL11;
	...
	?>
			...
			<tr>
				<td colspan="9"><b><?php echo $nombre_cliente; ?></b></td>
			</tr>
    <?php	
			...
			//Buscar Facturas
			$resSQL4="SELECT F.ID as ID, F.Moneda as Moneda, F.Ticket as Ticket, F.XFolio as XFolio, F.Creado as Creado, F.FechaRevision as FechaRevision,  
							F.zTotal as zTotal, F.CobranzaAbonado as CobranzaAbonado, F.CobranzaSaldo as CobranzaSaldo, F.Vence as Vence, 
							F.Comentarios as Comentarios, F.cfdfchhra as FechaTimbrado, F.DiasCredito as DiasCredito FROM ".$prefijobd."factura F, ".$prefijobd."oficinas O WHERE F.CobranzaSaldo > 0 
