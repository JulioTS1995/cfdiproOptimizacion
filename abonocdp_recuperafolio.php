...
$resSQL25 = "SELECT cfdmsgerror FROM basdb.".$prefijobd."abonos WHERE ID='".$idcdp."';";
...
$resSQL0 = "SELECT * FROM basdb.".$prefijobd."zzzlogtablas WHERE ID='".$idcdp."';";
...
if ($msgerrorcdp=='Folio previamente utilizado.'){  //Revisa si el folio esta previamente timbrado para recuperar desde forsedi
	...
	if ($Multi=='1'){
		$resSQL07 = "SELECT a.folio, b.Serie as cfdserie, b.seriefiscal, c.RFC AS RfcMulti FROM basdb.{$prefijobd}abonos a INNER JOIN basdb.{$prefijobd}oficinas b ON a.Oficina_RID = b.id LEFT JOIN basdb.{$prefijobd}emisores c ON a.Emisor_RID = c.id WHERE a.id=".$idcdp."";
		//echo $resSQL07;
		...
		while($rowSQL07 = mysqli_fetch_array($runSQL07)){
			...
			$seriecfd = $rowSQL07['cfdserie'];
		}
	} else {
		$resSQL07 = "SELECT a.folio, b.Serie as cfdserie,  b.seriefiscal FROM basdb.{$prefijobd}abonos a INNER JOIN basdb.{$prefijobd}oficinas b 
		ON a.Oficina_RID = b.id WHERE a.id=".$idcdp."";
		$runSQL07 = mysqli_query($cnx_cfdi2, $resSQL07);
		while($rowSQL07 = mysqli_fetch_array($runSQL07)){
			$Folio = $rowSQL07['folio'];
			$Serie = $rowSQL07['seriefiscal'];
	...
	if ($bandtimbrado>0){
		...
		//echo $RutaBMP;
		$RutaXml = rtrim($RutaXml, "\\/") . DIRECTORY_SEPARATOR . 'P'.$seriecfd.$Folio.'='.$Serie.'-'.$Folio.'.xml';
		...
		if (file_exists($RutaXml)){
			...
			$RutaBMP.= '\P'.$seriecfd.$Folio .'='. $Serie.'-'.$Folio .'.bmp';
			...
			cfdinoCertificadoSAT=".$nocertificadosat.", cfdiselloSAT='".$selloSAT."', cfdicbbArchivo='".$RutaBMP."' Where ID=".$idcdp.";"; 
			...
			echo "<H2 align='center'>Error No. 103 (FPT) Contacte a soporte.</H2>".$RutaXml;
			die;
		}
		
	}else{
		$begintrans = mysqli_query( $cnx_cfdi2, "BEGIN");
		$resSQL10 = "delete from basdb.".$prefijobd."zzzlogtablas Where id=".$idcdp."";
...
if ($noregistros<1) {
	...
	$resSQL05 = "SELECT * FROM basdb.".$prefijobd."zzzlogtablas WHERE ID='".$idcdp."';";
...
if ($error=='1'){
	...
	$begintrans = mysqli_query( $cnx_cfdi2, "BEGIN");
	$resSQL2 = "UPDATE basdb.".$prefijobd."zzzlogtablas SET vlogi=0 Where id=".$idcdp.";"; 
	//die $resSQL1;
	...
	if ($band_upd_log) {
		...
		$resSQLerror = "Update basdb.".$prefijobd."abonos Set cfdmsgerror='' Where id=".$idcdp."";
