...
function calcularTiempoTimbrado($Conteo) {
    $rangos = array(
        array(0,5,6), array(6,10,4), array(11,50,3), array(51,100,2),
        array(101,200,1.8), array(201,300,1.5), array(301,500,1.2),
        array(501,800,1), array(801,1000,0.7), array(1001,999999,0.5)
    );
    $tiempo = 1;
    foreach ($rangos as $r) {
...
/* ===== Datos del abono para nombres/rutas ===== */
$cfdicbbArchivo = ''; $CorreoCd=''; $SerieFiscal=''; $Folio=0; $Conteo=0;
if ($q = $cnx_cfdi3->prepare("    SELECT a.cfdicbbArchivo, b.CorreoCobranza, c.SerieFiscal, a.XFolio, a.Folio, COUNT(*) AS Conteo
    ...
    $q->execute();
    $q->bind_result($cfdicbbArchivo, $CorreoCd, $SerieFiscal, $XFolio, $Folio, $Conteo);
    $q->fetch();
    $q->close();
}
...
$urlPDFWeb = join_url($basePublica, 'P'.$XFolio.'='.$SerieFiscal.'-'.$Folio.'.pdf');
$urlXMLWeb = join_url($basePublica, 'P'.$XFolio.'='.$SerieFiscal.'-'.$Folio.'.xml');

/* ===== Velocidad de barra ===== */
$subEmbalaje = calcularTiempoTimbrado($Conteo);

/* ===== Lanzar EXE una sola vez (modo con/sin DirPHPPDF se decide después) ===== */
$cmd = "start /B \"\" \"C:\\TSRunCFDiWEB\\TSRuncfdiWEB.exe\" -d 6 -v " . escapeshellarg($iddocumento) . " -dbi " . escapeshellarg($prefijobd);
pclose(popen($cmd, "r"));
...
<style>
  ...
  .card{background:#fff;border:1px solid #e5e5e5;border-radius:10px;padding:16px;box-shadow:0 2px 8px rgba(0,0,0,.03)}
  .progress{height:26px;background: #ffffff;border:1px solid #e5e5e5;border-radius:999px;box-shadow:inset 0 0 0 999px #fff}
  .progress-bar{background:rgb(39, 126, 212); color: #111111;font-weight:bold;box-shadow:none}
  ...
  .btn-white{background:rgb(39, 126, 212);border:1px solid #ddd;color:#111}
  .btn-white:hover{background:rgb(78, 132, 185)}
  .muted{color:#666}
</style>
</head>
<body>
  <div class="container-limit">
    <h3>Timbrando su Complemento</h3>
...
function verificarUUID(){
  ...
  $.ajax({
    ...
    type: 'GET',
    cache: false,
    data: { id: CFG.id, prefijo: CFG.prefijo, tipoT: 'abonos', _t: Date.now() },
...
function verificarPDF(n){
  var maxN=12;
  $.ajax({
    url: CFG.pdfDL + (CFG.pdfDL.indexOf('?')>-1 ? '&' : '?') + '_t=' + Date.now(),
    type: 'HEAD',
    success: function(){
      renderDescargas();
