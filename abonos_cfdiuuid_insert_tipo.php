...
//Inicio la transaccion
	...
	$resSQL1 = "SELECT COUNT(*) as total FROM " . $prefijodb . "abonosuuidrelacionadosub WHERE FolioSub_RID = ".$idfactura." AND TipoRelacion='".$tiporelacion."'";
	...
	if($v_total == 0){
		...
		$sql = "INSERT INTO " . $prefijodb . "abonosuuidrelacionadosub (ID, cfdiuuidRelacionado, XFolio, FolioSub_RID, FolioSub_REN, FolioSub_RMA, BASTIMESTAMP, TipoRelacion, TipoRelacion2) VALUES (". $newid .", '". $cfdiuuid ."', '". $xfolio ."', ". $idfactura .", 'Abonos', 'FolioSubPartidas', '".$fecha."','".$tiporelacion."','".$tiporelacion2."')";
		
		mysql_query($sql,$cnx_cfdi);
		
		if($tiporelacion2 == '066'){
			//Actualizar Sustituida Por en Abono Seleccionada
			...
			//Actualizar campo TipoRelacion en Abono
			$sql_update2 = "UPDATE " . $prefijodb . "abonos SET 
			TipoRelacion = '".$tiporelacion."'
			WHERE ID = ".$idfactura;
			
			mysql_query($sql_update2,$cnx_cfdi);
			
			echo "<h1>Abono ".$xfolio."  Anexado con Exito a Abono: ".$facturaorigen."</h1>";
		} else {
			//Actualizar Sustituida Por en Abono Seleccionada
			$sql_update = "UPDATE " . $prefijodb . "abonos SET 
			...
			echo "<h1>Abono ".$xfolio."  Anexado con Exito al Abono: ".$facturaorigen."</h1>";
		}
		
		
	
	} else {
		echo "<h1>NOTA: El Abono: ".$facturaorigen." ya tiene cfdiuuid Relacionado, no es posible relacionar mas Abonos</h1>";
	}
