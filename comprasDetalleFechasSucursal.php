		...
		<div class="row">
			...
			<br>
				<div id="encabezadoform" style="text-align:center">
				  <h2 class="font-weight-bold" style="text-align: center;color:#0059b3; line-height: 100px;">Compras Detalle</h2>
				</div>
				<br>
				<center>
					<form method="post" action="comprasDetalleSucursal.php" enctype="multipart/form-data">
						...
						<div class="form-group">
							<div>
								...
								$resSQL = "SELECT ID,RazonSocial FROM ".$_GET["prefijodb"]."Proveedores ORDER BY RazonSocial";
								...
								{
									?>
										<option value='<?php echo $rowSQL['ID']; ?>'><?php echo $rowSQL['RazonSocial']; ?></option>
									<?php
						...
						<div>
							<label>Unidad:</label>
							<select class="form-control inputdefault" name="unidad" id="unidad" required aria-required="true">
							...
							mysqli_select_db($cnx_cfdi2,$database_cfdi);
									...
									$resSQL1 = "SELECT ID, Unidad FROM ".$_GET["prefijodb"]."Unidades ORDER BY Unidad";
								$runSQL1 = mysqli_query($cnx_cfdi2,$resSQL1);
								if (!$runSQL1) {//debug
									$mensaje  = 'Consulta no válida: ' . mysqli_error($cnx_cfdi2) . "\n";
									$mensaje .= 'Consulta completa: ' . $resSQL1;
									die($mensaje);
								}
									while ($rowSQL1 = mysqli_fetch_assoc($runSQL1))
									{
										?>
										<option value='<?php echo $rowSQL1['ID']; ?>'><?php echo $rowSQL1['Unidad']; ?></option>
									<?php
					...
					<div class="form-group">
						<div>
							...
							<select class="form-control inputdefault" name="producto" id="producto" required aria-required="true">
							...
							mysqli_select_db($cnx_cfdi2,$database_cfdi);
									...
									$resSQL11 = "SELECT ID, Codigo FROM ".$_GET["prefijodb"]."Productos ORDER BY Codigo";
								$runSQL11= mysqli_query($cnx_cfdi2,$resSQL11);
								if (!$runSQL11) {//debug
									$mensaje  = 'Consulta no válida: ' . mysqli_error($cnx_cfdi2) . "\n";
									$mensaje .= 'Consulta completa: ' . $resSQL11;
									die($mensaje);
								}
									while ($rowSQL11 = mysqli_fetch_assoc($runSQL11))
									{
										?>
										<option value='<?php echo $rowSQL11['ID']; ?>'><?php echo $rowSQL11['Codigo']; ?></option>
									<?php
...
<!--  https://tractosoft-espejo71.com/cfdipro/comprasDetalleFechasSucursal.php?prefijodb=OPTIMIZACION_&emisor=<<LoggedInUsuarios.Oficina.Emisor.ID>>&sucursal=<<LoggedInUsuarios.Sucursal.ID>>   -->