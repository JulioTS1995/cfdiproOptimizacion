...
$idFact  = $_POST['idFact']  ?? $_GET['idFact']  ?? null;

if (!$prefijo || !$usuario || !$idFact) {
...
$clientesCorreos = $cnx_cfdi2->query("SELECT ID, Correo, Nombre
    ...
    WHERE FolioSubCorreo_RID = (SELECT CargoAFactura_RID FROM {$prefijo}Factura WHERE ID = {$idFact})
");
...
if (isset($_POST['enviar'])) {
    ...
    $stmt2 = $cnx_cfdi2->prepare("SELECT f.cfdserie, f.cfdfolio, f.cfdfchhra as Fecha, f.Vence, f.TotalLetra as Total, c.RazonSocial, c.RFC,
        (SELECT xmldir FROM {$prefijo}SystemSettings LIMIT 1) AS dirDocumentos, 
        (SELECT VCHAR FROM {$prefijo}parametro WHERE ID2 = 943 LIMIT 1) AS Asunto,
        (SELECT MEMO FROM {$prefijo}parametro WHERE ID2 = 943 LIMIT 1) AS Cuerpo
        FROM {$prefijo}Factura f
        LEFT JOIN {$prefijo}clientes c ON c.ID = f.CargoAFactura_RID WHERE f.ID = ?");
    ...
    $dir = str_replace('\\', '/', $factura['dirDocumentos']);

    $nombreArchivo = 'C:/xampp/htdocs' . rtrim($dir, '/') . '/' .$factura['cfdserie'] . '-' . $factura['cfdfolio'];
    $folio = $factura['cfdfolio'];
    $xFolio = $factura['cfdserie'].$factura['cfdfolio'];
    $fechaFactura = $factura['Fecha'];
    $tipo = 'FACTURA DE VIAJE';
    $vencimiento = $factura['Vence'];
    $totalFactura = $factura['Total'];
    $nombreCliente = $factura['RazonSocial'];
    $rfcCliente = $factura['RFC'];
                          ...
                          <tr>
                            <td style="padding:0 18px;font-family:Arial, sans-serif;">
                              ...
                              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border:1px solid #e5e7eb;border-top:0;border-radius:0 0 6px 6px;overflow:hidden;">
                                ...
                                <tr>
                                  <td style="padding:8px 10px;font-size:12px;color:#374151;border-bottom:1px solid #e5e7eb;">Vencimiento:</td>
                                  <td style="padding:8px 10px;font-size:12px;color:#111827;border-bottom:1px solid #e5e7eb;">'.$h($vencimiento).'</td>
                                </tr>
                                <tr>
            ...
            switch ($adj) {
                ...
                case 'evidencias':
                    ...
                    $sql = "SELECT Evidencia_DOCDATA, Evidencia_DOCTYPE
                        FROM {$prefijo}FacturaEvidencias
                        WHERE ID IN (SELECT RID FROM {$prefijo}Factura_ref WHERE ID = {$idFact})
                    ";
...
<head>
  <meta charset="utf-8">
  <title>Enviar correo · Factura</title>
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <script>
    (function(){
  ...
  <div class="container">
    ...
    <div class="panel">
      ...
      <form method="post" action="">
        ...
        <div style="margin-top:14px;">
          ...
          <div class="list" style="margin-top:10px;">
            <?php while ($c = $clientesCorreos->fetch_assoc()): ?>
              <div class="row">
                <div class="left">
                  <input class="chkDest" type="checkbox" name="destinatarios[]" value="<?= htmlspecialchars((string)$c['Correo']) ?>">
                  <div style="min-width:0;">
                    <div class="name"><?= htmlspecialchars((string)$c['Nombre']) ?></div>
                    <div class="mail"><?= htmlspecialchars((string)$c['Correo']) ?></div>
                  </div>
                </div>
              </div>
            <?php endwhile; ?>
        ...
        <input type="hidden" name="idFact"  value="<?= htmlspecialchars($idFact) ?>">
