...
// Cargar usuarios 
$usuarios = array();
$sqlUser = "SELECT ID, Usuario FROM ".$prefijobd."logappmovil GROUP BY Usuario ORDER BY Usuario ASC;";
$resUser = mysqli_query($cnx_cfdi2, $sqlUser);
if ($resUser) {
...
<head>
  <meta charset="UTF-8">
  <title>Evidencias Bitacora Operador Filtros</title>
...
<body>
  <div class="container">
    <div class="header">
      <h1>Evidencias de Bitacora Operador</h1>
    ...
    <div class="panel">
      <form method="post" action="evidenciasBitacoraOperador.php">
        <div class="field">
          <label for="usuario">Usuario</label>
          <select name="usuario" id="usuario" required>
            <option value="0">Todos los Usuarios</option>
            <?php foreach($usuarios as $p){ ?>
              <option value="<?php echo $p['Usuario']; ?>"><?php echo htmlspecialchars($p['Usuario']); ?></option>
            <?php } ?>
          </select>
        </div>
