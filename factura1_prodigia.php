...
/* ===== Parámetros ===== */
if (!isset($_REQUEST['facturaid']) || $_REQUEST['facturaid']==='') { die("Falta id de la factura"); }
if (!isset($_REQUEST['prefijo']) || $_REQUEST['prefijo']==='') { die('Falta el prefijo de la base de datos'); }

$iddocumento = (int)$_REQUEST['facturaid'];
...
$DirPHPPDF = '0';
$xmldir = ''; $Servidor = '';
if ($st = $cnx_cfdi3->prepare("SELECT COALESCE(DirPHPPDF,'0'), COALESCE(xmldir,''), COALESCE(Servidor,''), COALESCE(Usuario,'') FROM {$prefijobd}systemsettings LIMIT 1")) {
    $st->execute();
    $st->bind_result($DirPHPPDF, $xmldir, $Servidor, $usuarioProdigia);
    $st->fetch();
    $st->close();
}
...
if ($q = $cnx_cfdi3->prepare("    SELECT a.cfdicbbArchivo, b.CorreoFactura, c.SerieFiscal, a.TipoViaje ,a.folio, Count(*) as Conteo FROM {$prefijobd}factura a 
                                  Inner Join {$prefijobd}clientes b On a.CargoAFactura_RID = b.ID 
                                  Inner Join {$prefijobd}oficinas c On a.Oficina_RID=c.ID 
                                  Inner Join {$prefijobd}facturassub d On a.ID = d.FolioSub_RID Where a.ID = ?")) {
    $q->bind_param("i", $iddocumento);
    $q->execute();
    $q->bind_result($cfdicbbArchivo, $CorreoFactura, $SerieFiscal, $tipoViaje ,$Folio, $Conteo);
    $q->fetch();
    $q->close();
}
...
if (!isset($usuarioProdigia) || empty($usuarioProdigia)) {
  # RUN
    $facturaid = escapeshellarg($_REQUEST["facturaid"]);
    $prefijo   = escapeshellarg($prefijobd);


    $cmd = "start /B \"\" \"C:\\TSRunCFDiWEB\\TSRuncfdiWEB.exe\" -d 1 -v $facturaid -dbi $prefijo";
  ...
  # PRODIGIA
    ...
    $url = "https://$host:8082/cfdipro/prodigia/timbrarFactura.php";
    
    // Armar query
    $queryParams = http_build_query([
        'facturaid' => $iddocumento,
        'prefijo' => $prefijobd
    ]);
...
<style>
  /* ===== Tema blanco minimal ===== */
  body{font-family:Arial,Helvetica,sans-serif;padding:24px;background: #f3f3f3ff;color: #111}
...
function verificarUUID(){
  ...
  $.ajax({
    ...
    type: 'GET',
    cache: false,
    data: { id: CFG.id, prefijo: CFG.prefijo, tipoT: 'factura', _t: Date.now() },
...
function enviarCorreo(){
  ...
  $.ajax({
    ...
    data: {
      ...
      prefijo: CFG.prefijo, correo: CFG.correo, iddocumento: CFG.id, _t: Date.now()
    },
