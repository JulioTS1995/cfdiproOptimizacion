...
mysql_set_charset('utf8', $cnx_cfdi);
mysql_query("SET NAMES 'utf8'", $cnx_cfdi);
...
$resSQL25 = "SELECT cfdmsgerror FROM basdb.".$prefijobd."factura WHERE ID='".$idfactura."';";
$runSQL25 = mysqli_query($cnx_cfdi2, $resSQL25);
while($rowSQL25 = mysqli_fetch_array($runSQL25)){
	$msgerrorfactura = $rowSQL25['cfdmsgerror'];
...
if ($msgerrorfactura=='Folio previamente utilizado.'){  //Revisa si el folio esta previamente timbrado para recuperar desde forsedi
	...
	if ($Multi=='1'){
		...
		$resSQL07 = "SELECT a.folio, b.seriefiscal FROM basdb.".$prefijobd."factura a inner join basdb.".$prefijobd."oficinas b 
		on a.Oficina_RID = b.id where a.id=".$idfactura."";
		$runSQL07 = mysqli_query($cnx_cfdi2, $resSQL07);
		while($rowSQL07 = mysqli_fetch_array($runSQL07)){
			$Folio = $rowSQL07['folio'];
			$Serie = $rowSQL07['seriefiscal'];
	...
	if ($bandtimbrado>0){
		...
		if (file_exists($RutaXml)){
			...
			//evita el uso de comillas simples en la variable
			$CadenaOriginal = mysql_real_escape_string($CadenaOriginal, $cnx_cfdi);
			...
			cfdinoCertificadoSAT=".$nocertificadosat.", cfdiselloSAT='".$selloSAT."', cfdicbbArchivo='".$RutaBMP."' Where ID=".$idfactura.";"; 
			...
			if ($band_upd_xml) {
				...
				echo '<H3> La factura se recupero correctamente (FOLIO PREVIAMENTE TIMBRADO) XML </H3> <br> <H3> Serie: </H3> <H3 style="color:#FF0000">', $seriefactura ,'</H3> <br> <H3> Folio: </H3> <H3 style="color:#FF0000">', $foliofactura, '</H3> <br> <H3> UUID: </H3> <H3 style="color:#FF0000">', $uuid , "</H3>";
				die;
			}else{
...
if ($procesado=='1'){
	...
	while(!feof($archivo)){
		...
		$traer = "";
		$traer = fgets($archivo);
		$traer = mb_convert_encoding($traer, 'UTF-8', 'ISO-8859-1');
...
if ($error=='1'){
	...
	if ($band_upd_log) {
		...
		$resSQLerror = "Update basdb.".$prefijobd."factura Set cfdmsgerror='' Where id=".$idfactura."";
