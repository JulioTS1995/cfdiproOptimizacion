...
if (file_exists($archivobuscado)){
	...
	if (file_exists($RutaIniAnt)){
		...
		//Como primera opción busca que no sea un Folio Previamente timbrado
		$resSQL99 = "SELECT cfdmsgerror FROM basdb.".$prefijobd."factura Where ID='".$idfactura."';";
		$runSQL99 = mysqli_query($cnx_cfdi2, $resSQL99);
		while($rowSQL99 = mysqli_fetch_array($runSQL99)){
			$foliotimbrado = $rowSQL99['cfdmsgerror'];
		}
		
		if (($foliotimbrado == 'Folio previamente utilizado.')){
			echo 'Entre a recuperar desde Folio';
			recuperafolioxml($prefijobd, $cnx_cfdi2, $idfactura, $cnx_cfdi, 2);
			//die;
		}	
		...
		if (file_exists($archivoerror)){
			...
			if (file_exists($RutaIniAntError)){
				...
				echo 'Entro a recuperar desde XML';
				recuperafolioxml($prefijobd, $cnx_cfdi2, $idfactura, $cnx_cfdi, 1);
				//die;
			}
		}	
	}
...
function recuperafolioxml($prefijobd, $cnx_cfdi2, $idfactura, $cnx_cfdi, $band){
	//echo 'Busca en forsedi si el archivo se timbro';
	//Busca si el archivo esta timbrado para recuperar desde el XML
	...
	if (($Multi=='1') and ($ExisteMulti=='1')){
		...
		$resSQL07 = "SELECT a.folio, b.seriefiscal FROM basdb.".$prefijobd."factura a inner join basdb.".$prefijobd."oficinas b on 
		a.Oficina_RID = b.id where a.id=".$idfactura."";
		$runSQL07 = mysqli_query($cnx_cfdi2, $resSQL07);
		while($rowSQL07 = mysqli_fetch_array($runSQL07)){
			$Rfc2 = $Rfc1;
			$Folio = $rowSQL07['folio'];
	...
	if ($bandtimbrado>0){
		...
		if (file_exists($RutaXml)){
			...
			$begintrans = mysql_query("BEGIN", $cnx_cfdi);
			$resSQL09 = "UPDATE basdb.".$prefijobd."factura SET cfdfolio=".$foliofactura.", cfdfchhra='".$fechafactura."', 
			cfdsellodigital='".$sellodigital."', cfdserie='".$seriefactura."', cfdanoaprobacion='".$AAprobacion."', 
			cfdnoaprobacion='".$NoAprobacion."', cfdnocertificado=".$nocertificado.", cfdiversion='".$version."', 
			cfdiuuid='".$uuid."', cfdifechaTimbrado='".$fechatimbrado."', cfdiselloCFD='".$selloCFD."', 
			cfdinoCertificadoSAT=".$nocertificadosat.", cfdcadenaoriginal='".$CadenaOriginal."', cfdisellocadenaoriginal='".$SelloCadenaOriginal."',
			cfdicbbDatos='".$CfdiCbbDatos."', cfdiselloSAT='".$selloSAT."', cfdicbbArchivo='".$RutaBMP."', cfdmsgerror='".$MsgError."' Where ID=".$idfactura.";"; 
			...
			if ($band_upd_xml) {
				...
				$endtrans = mysql_query("COMMIT", $cnx_cfdi);
				if ($band=='2'){
					echo '<H3> La factura se recupero correctamente FOLIO PREVIAMENTE TIMBRADO </H3> <br> <H3> Serie: </H3> <H3 style="color:#FF0000">', $seriefactura ,'</H3> <br> <H3> Folio: </H3> <H3 style="color:#FF0000">', $foliofactura, '</H3> <br> <H3> UUID: </H3> <H3 style="color:#FF0000">', $uuid , "</H3>";
					die;
				} else {
				...
				$endtrans = mysql_query("ROLLBACK", $cnx_cfdi);
				$error = "MySQL eor ".mysql_errno().": ".mysql_error()."\n<br>When executing:<br>\n$resSQL09\n<br>";
				echo $error;
				echo "<H2 align='center'>Error No. 107 Contacte a soporte.</H2>";
				die;
			}
		}
	} //Finaliza IF de la busqueda en la tabla de FORSEDI
} //Finaliza Proceso Recupera de XML
