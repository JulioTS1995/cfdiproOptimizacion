...
function obtenerSiguienteID($cnx_cfdi2) {
	...
	while ($fila_exp3=mysqli_fetch_array($res_03)){
		...
		if($id_remision_anexada > 0){ //IF_1
			...
			//echo $sql_04;
			$res_04=mysqli_query($cnx_cfdi2, $sql_04);
			while ($fila_exp4=mysqli_fetch_array($res_04)){
				...
				$rem_Operador2_REN = $fila_exp4['Operador2_REN'];
				$rem_Operador2_RID = $fila_exp4['Operador2_RID'];
				$rem_Operador2_RMA = $fila_exp4['Operador2_RMA'];
				if($rem_Operador2_RID > 0){
					$q_rem_Operador2_RID = "Operador2_RID=".$rem_Operador2_RID.",";
					$q_rem_Operador2_REN = "Operador2_REN='".$rem_Operador2_REN."',";
					$q_rem_Operador2_RMA = "Operador2_RMA='".$rem_Operador2_RMA."',";
				} else {
					$q_rem_Operador2_RID = "Operador2_RID=null,";
					$q_rem_Operador2_REN = "Operador2_REN=null,";
					$q_rem_Operador2_RMA = "Operador2_RMA=null,";
				}
				...
				$indicaciones=$fila_exp4['Indicaciones'];

				//FiguraTransporteTipo1

				$parteTrans1 = $fila_exp4['ParteTransporte1_RID'];
				if (isset($parteTrans1)) {
					//echo "Variable definida!!!";
					$operador2 = $fila_exp4['Operador2_RID'];
					...
					$q_unidad2 = ",
					Operador2_REN='Operadores',
					Operador2_RID='".$operador2."',
				...
				// Nuevos Campos 06/08/2024
				$rem_RemitenteResidenciaFiscal = $fila_exp4['RemitenteResidenciaFiscal'];
				$rem_DestinatarioResidenciaFiscal = $fila_exp4['DestinatarioResidenciaFiscal'];
				
				///////////////////////////////////////////////////////////////Nuevos Campos 19/11/2024
				//$rem_PesoBrutoVehicular = $fila_exp4['PesoBrutoVehicular'];
				//En consulta mysql
				//PesoBrutoVehicular=".$rem_PesoBrutoVehicular."
			...
			Moneda = '".$moneda."',
			RemitenteResidenciaFiscal='".$rem_RemitenteResidenciaFiscal."',
			DestinatarioResidenciaFiscal='".$rem_DestinatarioResidenciaFiscal."',
			...
			*/
			...
			mysqli_query($cnx_cfdi2,$mysql_update_factura);
			...
			if ($rem_Ticket != NULL && $rem_Ticket != ''){  //Verifica si el campo Ticket esta en vacío o en NULL
				...
				mysqli_query($cnx_cfdi2,$mysql_update_factura2);
				
			}
			/*3.1*/
			
			$querySelectRegimenAd="SELECT Regimen_RID FROM ".$prefijodb."RemisionesRegimenAduanero WHERE FolioSub_RID = $id_remision_anexada";
			$runsqlSelectRegimenAd = mysqli_query($cnx_cfdi2, $querySelectRegimenAd);
			if (!$runsqlSelectRegimenAd) {//debug
				$mensaje  = 'Consulta no valida [SelectRegimenAd]: ' . mysqli_error() . "\n";
				$mensaje .= 'Consulta completa: ' . $querySelectRegimenAd;
				//die($mensaje);
			}while ($rowsqlSelectRegimenAd = mysqli_fetch_assoc($runsqlSelectRegimenAd)){
				$queryInsertRegimenAd="INSERT INTO ".$prefijodb."FacturaRegimenAduanero (ID, Regimen_REN, Regimen_RID, Regimen_RMA, FolioSub_REN, FolioSub_RID, FolioSub_RMA)
					VALUES 
					('".obtenerSiguienteID($cnx_cfdi2)."', 'c_RegimenAduanero', '".$rowsqlSelectRegimenAd['Regimen_RID']."', NULL, 'Factura', '$id_factura', 'RegimenesAduaneros')";
				$runsqlInsertRegimenAd = mysqli_query($cnx_cfdi2, $queryInsertRegimenAd);
				if (!$runsqlInsertRegimenAd) {//debug
					$mensaje  = 'Consulta no valida [InsertRegimenAd]: ' . mysqli_error() . "\n";
					$mensaje .= 'Consulta completa: ' . $queryInsertRegimenAd;
					die($mensaje);
				}
		}
			
			echo "<h2>La Factura se Actualizo con Exito</h2>";
		} else {
