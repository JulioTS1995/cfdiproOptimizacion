...
mysqli_select_db($cnx_cfdi2,$database_cfdi);
	...
	//Borrar facturassub de la Factura *************************************
	
	//Buscar IDs de FacturaSub 
	$sql_01="SELECT * FROM ".$prefijodb."facturassub WHERE FolioSub_RID = ".$id_factura;
	//echo $sql_01;
	$res_01=mysqli_query($cnx_cfdi2, $sql_01);
	while ($fila_exp1=mysqli_fetch_array($res_01)){
		$id_facturasub = $fila_exp1['ID'];
		
		//echo "Eliminar FacturaSub ID: ".$id_facturasub." <br>";
		
		mysqli_query($cnx_cfdi2,"DELETE FROM ".$prefijodb."facturassub WHERE ID = ".$id_facturasub);

	}
	
	//Buscar ID  de Remisiones Anexadas a la Factura
	$sql_03="SELECT * FROM ".$prefijodb."facturasdetalle WHERE FolioSubDetalle_RID = ".$id_factura." ORDER BY ID ASC";
	//echo $sql_03;
	$res_03=mysqli_query($cnx_cfdi2, $sql_03);
	$num_rows = mysqli_num_rows($res_03);
	//echo mysql_num_rows($res_03);
	if($num_rows > 0){
		while ($fila_exp3=mysqli_fetch_array($res_03)){
			$id_remision_anexada = $fila_exp3['Remision_RID'];
			
			//Buscar registros de RemisionesSub de la Remision
			$sql_04="SELECT * FROM ".$prefijodb."remisionessub WHERE FolioSub_RID = ".$id_remision_anexada;
				//echo $sql_04;
				$res_04=mysqli_query($cnx_cfdi2,$sql_04);
				$num_rows2 = mysqli_num_rows($res_04);
				if($num_rows2 > 0){
					while ($fila_exp4=mysqli_fetch_array($res_04)){
						...
						$sql_insert1="INSERT INTO ".$prefijodb."facturassub 
						(ID, 
						BASVERSION, 
						BASTIMESTAMP, 
						Documentador, 
						...
						mysqli_query($cnx_cfdi2,$sql_insert1);

						$queryShowTables = "SHOW TABLES LIKE '".$prefijodb."RepartoSub'";
						$resultShowTables = mysqli_query($cnx_cfdi2, $queryShowTables);
						
						if (mysqli_num_rows($resultShowTables) > 0) {
						$queryRepartoRemSub = "SELECT * FROM ".$prefijodb."RepartoSub WHERE FolioRemisionSub_RID='$remisionsub_id';"; 
						$runsqlRepartoRemSub = mysqli_query($cnx_cfdi2, $queryRepartoRemSub);
						if (!$runsqlRepartoRemSub) {//debug
							$mensaje  = 'Consulta no válida [queryRepartoRemSub]: ' . mysqli_error() . "\n";
							//$mensaje .= 'Consulta completa: ' . $queryRepartoRemSub;
							die($mensaje);
						}
						while ($rowsqlRepartoRemSub = mysqli_fetch_assoc($runsqlRepartoRemSub)){
							$idOrigen= $rowsqlRepartoRemSub['IDOrigen'];
							$idDestino= $rowsqlRepartoRemSub['IDDestino'];
							$cantidad= $rowsqlRepartoRemSub['Cantidad'];
							...
							$queryP = "INSERT INTO ".$prefijodb."RepartoSub
							(ID, IDOrigen, IDDestino, Cantidad, FolioFacturaSub_REN,FolioFacturaSub_RID) 
							values
							('$newid2','$idOrigen','$idDestino','$cantidad','FacturasSub','$newid');";
							//$newquery=$queryP;
							...
							if (!$runP) {//debug
								$mensaje  = 'Error en consulta. Informe a soporte: [RepartoSub]' . mysqli_error() . "\n";
		...
		echo "<h2>La Factura no cuenta con Remisones Anexadas </h2>";
	}
