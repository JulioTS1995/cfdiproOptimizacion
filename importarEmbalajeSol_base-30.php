...
if(isset($_POST["submit"]))//cuando se presiona el votor enviar... 
{
 ...
 //Borrar solicitudessub que tenga la Solicitud antes de actualizar (FolioSub_RID)
 
 $query2 = "DELETE FROM ".$prefijobd."Solicitudessub WHERE FolioSub_RID = ".$idRem;
 //echo $query2;
 ...
 if($_FILES['file']['name'])
 {
  ...
  if($filename[1] == 'csv')
  {
   ...
   while(($data = fgetcsv($handle,10000,","))!==FALSE)//empieza a leer los datos,
   {
                ...
                $queryP = "INSERT INTO ".$prefijobd."Solicitudessub(ID,Mt3,Modificado,PesoVolumen,PesoEstimado,PesoCobrar,BASTIMESTAMP,BASVERSION,Documentador,FolioSub_REN, FolioSub_RID, Cantidad, Embalaje, BL, Pedimento, Tipo, Peso,
                 ...
                 ('$newid',0,'$time',0,0,'$item6','$time',0,'Tractosoft','Solicitudes', '$idRem', '$item1', '$item2', '$item3', '$item4', '$item5', '$item6', '$item7', '$item8', 
