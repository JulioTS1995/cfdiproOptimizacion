...
if(isset($_POST["submit"]))//cuando se presiona el votor enviar... 
{
 if($_FILES['file']['name'])
 {
  ...
  if($filename[1] == 'xml')
  {
	...
	//die($target_file);
    //$uploadOk = 1;
    //$dir_to_search = $_FILES["file"];
    //$xmlfile = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
	//die(implode(",",$dir_to_search));
    //$aux=file_get_contents($target_file);
	//die($aux);
    $xml2 = simplexml_load_file($target_file);
    //echo $xml2->root->UbicacionRemitente->RFCDestinatario , PHP_EOL;
    //die($xml2);
    ...
    /*foreach ($xml2-> as $Root){
        echo 'rfc: '.$Root->UbicacionRemitente->RFCDestinatario.'<br>';
        echo 'nom desti: '.$Root->UbicacionRemitente->NombreDestinatario.'<br>';
        echo 'res fisc: '.$Root->UbicacionRemitente->ResidenciaFiscalDestinatario.'<br>';
        echo 'calle: '.$Root->UbicacionRemitente->CalleDestinatario.'<br>';
        echo 'colonia: '.$Root->UbicacionRemitente->ColoniaDestinatario.'<br>';*/
   ...
   /*while(($data = fgetcsv($handle,10000,","))!==FALSE)//empieza a leer los datos,
   {
                ...
                $queryP = "INSERT INTO ".$prefijobd."remisionessub(ID,Mt3,Modificado,PesoVolumen,PesoEstimado,PesoCobrar,BASTIMESTAMP,BASVERSION,Documentador,FolioSub_REN, FolioSub_RID, Cantidad, Embalaje, BL, Pedimento, Tipo, Peso,
                 ...
                 ('$newid',0,'$time',0,0,'$item6','$time',0,'Tractosoft','Remisiones', '$idRem', '$item1', '$item2', '$item3', '$item4', '$item5', '$item6', '$item7', '$item8', 
                 ...
                 '$item18', 'c_MaterialPeligroso', '$MaterialPeligroso', 'c_FraccionArancelaria', '$FraccionArancelaria', '$item21','$item22');";
				 //$newquery=$queryP;
 ...
 <body>  
  ...
  <form method="post" enctype="multipart/form-data">
    <div class="form-group">
                <label>Cliente:</label>
                <select class="form-control inputdefault" name="cliente" id="cliente">
        ...
        <div class="form-group">
                <label>Oficina:</label>
                <select class="form-control inputdefault" name="oficina" id="oficina">
                    <option value='0'>Selecciona Oficina</option>
                <?php
        
        //require_once('cnx_cfdi.php');
        //mysql_select_db($database_cfdi, $cnx_cfdi);
                    $resSQL = "SELECT ID,Serie as Oficina FROM ".$prefijobd."Oficinas WHERE Sucursal_RID = ".$sucursal." ORDER BY Oficina";
            $runSQL = mysql_query($resSQL, $cnx_cfdi);  
                while ($rowSQL = mysql_fetch_assoc($runSQL))
                {
                    ?>
                    <option value='<?php echo $rowSQL['ID']; ?>'><?php echo $rowSQL['Oficina']; ?></option>
                <?php
