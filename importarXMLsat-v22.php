 ...
 if($_FILES['file']['name'])
 {
  ...
  if($filename[1] == ('xml')||('XML')){
    ...
    foreach ($xml2->xpath('//cfdi:Receptor') as $Receptor) {
        ...
        foreach ($xml2->xpath('//cfdi:Conceptos//cfdi:Concepto') as $Concepto) {
			...
			(float)$Retencion[$cont3]=(float)$Retencion[$cont3]*100;
                   ...
                   $queryP = "INSERT INTO ".$prefijobd."FacturaPartidas(ID,Cantidad,prodserv33,claveunidad33,Detalle,Importe,PrecioUnitario,IVAImporte,IVA,RetencionImporte,Retencion,FolioSub_REN,FolioSub_RID,ConceptoPartida,Tipo,Subtotal1,Subtotal) values
                    ('$new2id','$cantidad','$claveProdServ','$claveUnidad','$descripcion','$importe','$valorUnitario','".$importeIVA[$cont3]."','".$IVA[$cont3]."','".$importeRetencion[$cont3]."','".$Retencion[$cont3]."','Factura','$newid','FLETE','Flete','$importe','$importe');";
						//die($queryP);
    ...
    //echo $key.": ".$value."</br>";
                ...
                RemitenteReferencia,DestinatarioReferencia,CitaCarga,DestinatarioCitaCarga,Unidad_REN,Unidad_RID,ConfigAutotranporte_REN,ConfigAutotranporte_RID,cfdiselloSAT,Operador_REN,Operador_RID,TipoViaje,Remolque_REN,Remolque_RID,Remolque2_REN,Remolque2_RID,Comentarios,xRetencion,xIVA) values
                ...
                'Unidades','".$Rem1."','Unidades','".$Rem2."','$folio','$retOficina','$ivaOficina');";
				 //$newquery=$queryP;
                $queryP=str_replace("''","NULL",$queryP);
                $queryP=str_replace("á","a",$queryP);
                $queryP=str_replace("é","e",$queryP);
                ...
                echo "<script>alert('Importacion Exitosa, se creo la Factura DIR:".$target_file."');</script>";//Imprime exito
   }
