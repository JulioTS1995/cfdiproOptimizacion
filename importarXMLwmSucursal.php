 ...
 <body>  
  ...
  <form method="post" enctype="multipart/form-data">
    <div class="form-group">
        ...
        //require_once('cnx_cfdi.php');
        //mysql_select_db($database_cfdi, $cnx_cfdi);
                    $resSQL = "SELECT ID,Operador FROM ".$prefijobd."Operadores WHERE Estatus = 'Activo' AND Sucursal_RID = ".$sucursal." ORDER BY Operador";
            $runSQL = mysql_query($resSQL, $cnx_cfdi);  
                while ($rowSQL = mysql_fetch_assoc($runSQL))
                {
                    ?>
        ...
        //require_once('cnx_cfdi.php');
        //mysql_select_db($database_cfdi, $cnx_cfdi);
                    $resSQL = "SELECT ID,Unidad FROM ".$prefijobd."Unidades WHERE Activa = 'Activa' AND Sucursal_RID = ".$sucursal." ORDER BY Unidad";
            $runSQL = mysql_query($resSQL, $cnx_cfdi);  
                while ($rowSQL = mysql_fetch_assoc($runSQL))
                {
                    ?>
