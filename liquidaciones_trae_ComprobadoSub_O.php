...
$resSQL="SELECT DISTINCT
GS.Cantidad,
GS.ConceptoLiq_RID,
GS.ID,
(SELECT XFolio FROM ".$prefijobd."Remisiones WHERE ID = G.Remision_RID) AS FolioRemision
FROM
...
".$prefijobd."LiquidacionesGastos AS LG
WHERE
G.Liquidacion = '".$xfolioLiq."'
    AND G.TipoVale='Deposito'
    AND GVR.ID = G.ID
    AND GVR.RID = GS.ID
    AND LG.GastosLiq_RID = G.ID
    AND LG.FolioSubGastos_RID = LS.FolioSub_RID ;";
	//die($resSQL);
...
while ($rowSQL=mysqli_fetch_array($runSQL)){
    ...
    (ID, FolioSub_REN, FolioSub_RID, Deposito, Concepto_REN,
     Concepto_RID, FolioLiqSub_REN, FolioLiqSub_RID, RemisionXFolio, Autorizado, ConceptoTxt) VALUES 
     ('".$newID."','Liquidaciones',(SELECT ID FROM ".$prefijobd."Liquidaciones WHERE XFolio = '".$xfolioLiq."'),
     '".$cantidad."','ConceptosLiquidaciones','".$conceptoID."','LiquidacionesSub','".$liqSubID."', '".$xfolioRemision."','0',
     (SELECT Concepto FROM ".$prefijobd."ConceptosLiquidaciones WHERE ID = '".$conceptoID."'))";*/
	 //die($resSQLInsert);


     $resSQLInsert="INSERT INTO ".$prefijobd."LiquidacionesComprobadoSub
    (ID, FolioSub_REN, FolioSub_RID, Deposito, Concepto_REN,
     Concepto_RID, RemisionXFolio, Autorizado, Diferencia) VALUES 
     ('".$newID."','Liquidaciones',(SELECT ID FROM ".$prefijobd."Liquidaciones WHERE XFolio = '".$xfolioLiq."'),
     '".$cantidad."','ConceptosLiquidaciones','".$conceptoID."', '".$xfolioRemision."','0',".$cantidad.")";
	 //die($resSQLInsert);
