<?php
    ...
    die("Falta ID de Requisiciones Compra");
	}
	
	$id_requisicion_compra = $_GET['id'];
	...
	if ($pos === false) {
		$vlprefijodb = $vlprefijodb . "_";
			...
			$resSQL1="SELECT * FROM ".$vlprefijodb."requisicioncompra WHERE ID = ".$id_requisicion_compra;			
    		$runSQL1 = mysql_query($resSQL1, $cnx_cfdi);
    		while($rowSQL1 = mysql_fetch_assoc($runSQL1)){
    			$oficina_id = $rowSQL1['OficinaODC_RID'];
				...
				$rubro_id = $rowSQL1['RubroODC_RID'];
				$proveedor_id = $rowSQL1['ProveedorOC_RID'];
			...
			$mensaje = '
				...
				<h2>Requisición de Compra: '.$xfolio.' </h2>
			...
			//Buscar RDC Sub
			$resSQL7="SELECT * FROM ".$vlprefijodb."requisicioncomprassub WHERE FolioSub_RID = ".$id_requisicion_compra;			
			...
			//echo $resSQL4;
				...
				$mail->Subject = 'Notificacion de Requisiciones de Compra: '.$xfolio;
				$mail->Body = $mensaje;
				...
				if($mail->send() == false){
					...
					echo "<br>";
					echo "<hr>";
					echo "<h2>La notificacion de la Requisicion de Compra: ".$xfolio."  se envio con Exito</h2>";
					echo "<hr>";
				}
