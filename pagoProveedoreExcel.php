...
$whereMoneda = "";
if ($moneda === 'PESOS') {
    $whereMoneda = " AND IFNULL(ps.tc_max,1) <= 1 ";
...
$sql = "SELECT
        ...
        LEFT JOIN (
          SELECT
            FolioSubPago_RID AS PagoID,
            MAX(IFNULL(TipoCambio,1)) AS tc_max
          FROM {$prefijobd}PagosSub
...
function printTotalProveedorExcel($moneda, $sumPesos, $sumDol, $sumDolEnPesos, $countPagos){
    ...
    if ($moneda === 'AMBOS') {
        ...
        echo '<td colspan="2" style="text-align:left;">TOTAL PROVEEDOR</td>';
        echo '<td style="text-align:right;">'.intval($countPagos).' pago(s)</td>';
        echo '<td colspan="2" style="text-align:right;">$ '.number_format($solo,2).'</td>';
    }
...
echo '<td colspan="5">Reporte Pago a Proveedores · Periodo: '.h($fecha_inicio).' al '.h($fecha_fin).' · Moneda: '.h($moneda).'</td>';
echo '</tr>';
...
while ($row = mysqli_fetch_assoc($res)) {
    ...
    $provId = (int)$row['ProveedorID'];          // FIX CLAVE: INT para agrupar bien
    ...
    if ($provActual === null || $provActual !== $provId) {
        ...
        if ($provActual !== null) {
            ...
            // fila separadora
            echo '<tr><td colspan="5" style="background:#ffffff;height:6px;"></td></tr>';
        ...
        // header proveedor
        echo '<tr style="font-weight:bold;background:#eef4ff;">';
        echo '<td colspan="5">'.h($provNombre).' (ID: '.intval($provId).')</td>';
        echo '</tr>';
    ...
    echo '<tr>';
    echo '<td>'.h($provNombre).'</td>';                 // en excel conviene repetir para filtros
...
if (!$hayFilas) {
    echo '<tr><td colspan="5" style="color:#666;">Sin resultados con los filtros actuales.</td></tr>';
}
