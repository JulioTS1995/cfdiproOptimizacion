...
if (isset($_POST["procesar"]) && $_POST["procesar"]==1){
	...
	if ($_POST["opciones"] == "2")
	{
		...
		$qryabonos = "SELECT a.FechaAplicacion, b.id";
		$qryabonos = $qryabonos." FROM ".$prefijobd."abonossub as a, ".$prefijobd."abonos as b";
		$qryabonos = $qryabonos." WHERE a.FechaAplicacion>='".$_REQUEST["fechaini"]."' AND a.FechaAplicacion<='" . $_REQUEST["fechafin"]."'";
		
		$qryabonos = $qryabonos." AND a.FolioSub_RID = b.id";

		$qryabonos = $qryabonos." GROUP BY a.FechaAplicacion, b.id ORDER BY a.FechaAplicacion ";
...
//			$qryabonossub = $qryabonossub.", (SELECT d.xfolio FROM ".$prefijobd."factura as d WHERE d.ID=a.AbonoFactura_RID) as factura";
			...
			$qryabonossub = $qryabonossub." AND a.FolioSub_RID = '".$rowabonos["id"]."'";
...
//		$qryabonos = $qryabonos.", (SELECT d.xfolio FROM ".$prefijobd."factura as d WHERE d.ID=a.AbonoFactura_RID) as factura";
		...
		$qryabonos = $qryabonos."  ORDER BY d.XFolio, a.FechaAplicacion";
		...
		while ($rowabonos = mysql_fetch_array($resultqryabonos))
		{
		echo "Opcion 2:</BR>".$rowabonos["Fecha"].":-:";
