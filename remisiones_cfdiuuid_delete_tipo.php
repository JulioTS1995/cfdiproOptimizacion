...
//Inicio la transaccion
	...
	//Buscar XFolio de Remision PPAL 
	
	$resSQL02 = "SELECT * FROM " . $prefijobd . "remisionesuuidrelacionadosub WHERE ID = ".$idfacturasub;
	...
	if($tiporelacion2=='066'){
		$resSQL01 = "SELECT * FROM " . $prefijobd . "remisiones WHERE ID = ".$idfoliosub;
		...
		//FIN Buscar XFolio de Remision PPAL 
			
		//Buscar ID en Remisiones para actualizar RelacionadoPor
		$resSQL00 = "SELECT * FROM " . $prefijobd . "remisiones WHERE XFolio = '".$xfoliosub."'";
		...
		$sql_update = "UPDATE " . $prefijobd . "remisiones SET 
			RelacionadoPor = ''
			WHERE ID = ".$idremision;
		...
		$sql_delete = "DELETE FROM " . $prefijobd . "remisionesuuidrelacionadosub 
			WHERE ID = ".$idfacturasub;
			
		mysql_query($sql_delete,$cnx_cfdi);
		
		
		echo "<h2>Remision Relacionada: <b>".$xfoliosub."</b> Se removio de la Remision: <b>".$xfolio2."</b></h2>";
	}else {
				$resSQL01 = "SELECT * FROM " . $prefijobd . "remisiones WHERE ID = ".$idfoliosub;
		$runSQL01 = mysql_query($resSQL01, $cnx_cfdi);
		$rowSQL01 = mysql_fetch_assoc($runSQL01);
		...
		//Buscar ID en Remision con XFolio para actualizar cfdiSustituidaPor
		$resSQL00 = "SELECT * FROM " . $prefijobd . "remisiones WHERE XFolio = '".$xfoliosub."'";
		$runSQL00 = mysql_query($resSQL00, $cnx_cfdi);
		$rowSQL00 = mysql_fetch_assoc($runSQL00);
		do { 
		...
		//Actualizar SustituidaPor en Remision Seleccionada
		$sql_update = "UPDATE " . $prefijobd . "remisiones SET 
			cfdiSustituidaPor = '',
			TipoRelacion = ''
			WHERE ID = ".$idfactura;
		...
		$sql_update2 = "UPDATE " . $prefijobd . "remisiones SET 
			TipoRelacion = ''
			WHERE ID = ".$idfoliosub;
	...
	//Limpiar campo TipoRelacion de la Remision
	$res55 = "SELECT * FROM " . $prefijobd . "remisionesuuidrelacionadosub WHERE FolioSub_RID = ".$idfoliosub;
	...
	if($total_reg > 0){
		...
		$sql_update55 = "UPDATE " . $prefijobd . "remisiones SET 
