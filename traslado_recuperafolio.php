...
$nombrebat = "traslado_recuperafolio.bat";
...
$resSQL25 = "SELECT cfdmsgerror FROM basdb.".$prefijobd."remisiones WHERE ID='".$idrem."';";
...
$resSQL0 = "SELECT * FROM basdb.".$prefijobd."zzzlogtablas WHERE ID='".$idrem."';";
...
if ($msgerrorcdp=='Folio previamente utilizado.'){  //Revisa si el folio esta previamente timbrado para recuperar desde forsedi
	...
	if ($Multi=='1'){
		$resSQL07 = "SELECT a.folio, b.seriefiscal, c.RFC AS RfcMulti FROM basdb.{$prefijobd}remisiones a INNER JOIN basdb.{$prefijobd}oficinas b ON a.Oficina_RID = b.id LEFT JOIN basdb.{$prefijobd}emisores c ON a.Emisor_RID = c.id WHERE a.id=".$idrem."";
		//echo $resSQL07;
		...
		$resSQL07 = "SELECT a.folio, b.seriefiscal FROM basdb.{$prefijobd}remisiones a INNER JOIN basdb.{$prefijobd}oficinas b 
		ON a.Oficina_RID = b.id WHERE a.id=".$idrem."";
		$runSQL07 = mysqli_query($cnx_cfdi2, $resSQL07);
		while($rowSQL07 = mysqli_fetch_array($runSQL07)){
			$Folio = $rowSQL07['folio'];
			$Serie = $rowSQL07['seriefiscal'];
	...
	if ($bandtimbrado>0){
		...
		if (file_exists($RutaXml)){
			...
			$resSQL09 = "UPDATE basdb.".$prefijobd."remisiones SET cfdfolio=".$foliofactura.", cfdfchhra='".$fechafactura."', 
			...
			cfdinoCertificadoSAT=".$nocertificadosat.", cfdiselloSAT='".$selloSAT."', cfdicbbArchivo='".$RutaBMP."' Where ID=".$idrem.";"; 
			...
			if ($band_upd_xml || isset($uuid)) {
				...
				echo '<H2> La Carta Porte Traslado se timbro correctamente <br> Ya no mandes timbrar, solo actualiza (FOLIO PREVIAMENTE TIMBRADO)  </H2> <br> <H2> Serie: </H2> <H2 style="color:#FF0000">', $seriefactura ,'</H2> <br> <H2> Folio: </H2> <H2 style="color:#FF0000">', $foliofactura, '</H2> <br> <H2> UUID: </H2> <H2 style="color:#FF0000">', $uuid , "</H2>";
				die;
			}else{
		...
		$begintrans = mysqli_query( $cnx_cfdi2, "BEGIN");
		$resSQL10 = "delete from basdb.".$prefijobd."zzzlogtablas Where id=".$idrem."";
...
if ($noregistros<1) {
	...
	$resSQL05 = "SELECT * FROM basdb.".$prefijobd."zzzlogtablas WHERE ID='".$idrem."';";
				...
				if ($bandtimbrado>0){
					...
					if (file_exists($RutaXml)){
						...
						if ($band_upd_xml) {
							//Se hizo el update sin problemas
							$endtrans = mysqli_query( $cnx_cfdi2, "COMMIT");
							echo '<H2> La Carta Porte Traslado se timbro correctamente, NO LO TIMBRES DE NUEVO, solo actualiza </H2> <br> <H2> Serie: </H2> <H2 style="color:#FF0000">', $seriefactura ,'</H2> <br> <H2> Folio: </H2> <H2 style="color:#FF0000">', $foliofactura, '</H2> <br> <H2> UUID: </H2> <H2 style="color:#FF0000">', $uuid , "</H2>";
							die;
						}else{
							$endtrans = mysqli_query( $cnx_cfdi2, "ROLLBACK");
							$error = "MySQL error ".mysqli_errno().": ".mysqli_error()."\n<br>When executing:<br>\n$resSQL09\n<br>";
...
if ($procesado=='1'){
	...
	while(!feof($archivo)){
		...
		if (strstr($traer, "sellocadenaoriginal")){
			...
			if ($temporal=="sellocadenaoriginal="){
				...
				$VselloCadenaOriginal = asegurar_utf8($VselloCadenaOriginal);
		...
		if (strstr($traer, "cadenaOriginal")){
			...
			if ($temporal=="cadenaOriginal="){
				...
				$Vcadenaoriginal = asegurar_utf8($Vcadenaoriginal);
	...
	if ($band_upd_timbrado) {
		//Se hizo el update sin problemas
		$endtrans = mysqli_query( $cnx_cfdi2, "COMMIT");
		echo '<H2>  La Carta porte Traslado se timbro correctamente, NO LO TIMBRES DE NUEVO, solo actualiza </H2> <br> <H2> Serie: </H2> <H2 style="color:#FF0000">', $Vserie ,'</H2> <br> <H2> Folio: </H2> <H2 style="color:#FF0000">', $Vfolio, '</H2> <br> <H2> UUID: </H2> <H2 style="color:#FF0000">', $Vuuid , "</H2>";
...
if ($error=='1'){
	...
	$begintrans = mysqli_query( $cnx_cfdi2, "BEGIN");
	$resSQL2 = "UPDATE basdb.".$prefijobd."zzzlogtablas SET vlogi=0 Where id=".$idrem.";"; 
	//die $resSQL1;
	...
	if ($band_upd_log) {
		//Se hizo el update sin problemas
		$endtrans = mysqli_query( $cnx_cfdi2, "COMMIT");
		echo '<H2>Se recupero el folio, no olvide corregir su error: </H2> <H2 style="color:#FF0000">', $facerror, "</H2><H2> Antes de volver a timbrar la factura</H2>";
		
		$resSQLerror = "Update basdb.".$prefijobd."remisiones Set cfdmsgerror='' Where id=".$idrem."";
		$runSQLerror = mysqli_query($cnx_cfdi2, $resSQLerror);
		
		//echo "<H2> La factura se recupero correctamente </H2>";
	}else{
		$endtrans = mysqli_query( $cnx_cfdi2, "ROLLBACK");
		//echo $resSQL1;
		$error = "MySQL error ".mysqli_errno().": ".mysqli_error()."\n<br>When executing:<br>\n$resSQL2\n<br>";
